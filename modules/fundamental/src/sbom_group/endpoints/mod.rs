...
pub async fn list(
    ...
    Ok(HttpResponse::Ok().json(
        service
            .list_groups(search, paginated, params.totals, &tx)
            .await?,
    ))
}
...
pub async fn get_group(
    ...
    let tx = db.begin_read().await?;
    let group = service
        .get_group(*id, params.children, params.totals, &tx)
        .await?;
...
pub async fn get_by_path(
    ...
    let tx = db.begin_read().await?;
    let group = service
        .get_group_by_path(&path, params.children, params.totals, &tx)
        .await?;
...
pub async fn delete_group(
    ...
    let revision = if let Some(if_match) = req.headers().get("If-Match") {
        ...
        let group = service.get_group(*id, false, false, &tx).await?;
        match group {
